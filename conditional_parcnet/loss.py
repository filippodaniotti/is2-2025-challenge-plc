...
class MultiResolutionSTFTLoss(nn.Module):
    ...
    def __init__(
        ...
        assert (
            len(fft_sizes) == len(win_lengths) == len(hop_sizes)
        ), "Multi-resolution params must have the same length"
        ...
        for n_fft, wlen, hop in zip(fft_sizes, win_lengths, hop_sizes):
            self.loss_functions.append(SingleResolutionSTFTLoss(n_fft, wlen, hop))
